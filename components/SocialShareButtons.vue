  ...
  ul {
    ...
    li {
      ...
      a {
        ...
        border-radius: 3px;
        color: $white-text;
        transition: filter, color 100ms ease-in-out;
        &.twitter {
...
<style lang="scss">
// unscoped so it applies to content dynamically generated with v-html
div.socialshare {
  ul {
    li {
      a {
...
<template>
  <div class="socialshare">
    <span class="label">{{ `Share this ${options.label}:` }}</span>
    <ul>
      <li v-for="(service, serviceIndex) in options.services" :key="`share-${serviceIndex}`">
        <a
          :href="availableServices[service].baseLink + encodeParams(availableServices[service].urlParams)"
          :title="availableServices[service].tooltip"
          v-html="availableServices[service].iconHtml"
          :class="service"
          target="_blank"
          rel="noopener"
        ></a>
    ...
    return {
      availableServices: {
        twitter: {
          ...
          urlParams: { text: this.options.text, url: this.options.url },
        ...
        facebook: {
          ...
          urlParams: { u: this.options.url },
          // windowParams: "width=580,height=296",
          tooltip: "Share on Facebook",
          iconHtml:
            '<svg aria-hidden="true" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 264 512"><path fill="currentColor" d="M76.7 512V283H0v-91h76.7v-71.7C76.7 42.4 124.3 0 193.8 0c33.3 0 61.9 2.5 70.2 3.6V85h-48.2c-37.8 0-45.1 18-45.1 44.3V192H256l-11.7 91h-73.6v229"></path></svg>'
        },
        email: {
          baseLink: "mailto:",
          urlParams: {
            subject: `Check out this ${this.options.label} from ${this.options.name}`,
            body:
              this.options.text +
              "\n\n" +
              "Read the rest at: " +
        ...
        reddit: {
          ...
          urlParams: { url: this.options.url, title: this.options.title },
          // windowParams: "",
        ...
        tumblr: {
          ...
          urlParams: {
            canonicalUrl: this.options.url,
            title: this.options.title,
            caption: this.options.text
          },
          // windowParams: "",
